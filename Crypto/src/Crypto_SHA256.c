...
#error "Version of Crypto_SHA256.c does not match the version of the module!"
#endif
...
/* definition logical function macros */
...
static void Crypto_SHA256Compress(hash_state * md, uint8 *buf);
...
static void Crypto_SHA256Compress(hash_state * md, uint8 *buf)
{
  ...
  for (i = 0; i < 16; i++) {
    W[i] = BUtil_Get32BitBigEndian(buf + (4*i));
  }
...
/**
 * \brief  This function performs the main algorithm of the SHA256 Hash process.
 *
 * \global_r  --
 * \global_w  --
 * \global_rw --
...
{
  Crypto_ReturnType ReturnValue = CRYPTO_E_NOK;
  uint32 n;
  ...
  if((md->sha256.curlen < sizeof(md->sha256.buf))) {
    while (inlen > 0) {
      if (md->sha256.curlen == 0 && inlen >= 64) {
         Crypto_SHA256Compress (md, (uint8 *)in);
         md->sha256.length += 512;
         in += 64;
         inlen -= 64;
      } else {
         n = BUtil_MIN(inlen, (64 - md->sha256.curlen));
         memcpy(md->sha256.buf + md->sha256.curlen, in, (size_t)n);
         md->sha256.curlen += n;
         in += n;
         inlen -= n;
         if (md->sha256.curlen == 64) {
            Crypto_SHA256Compress (md, md->sha256.buf);
            md->sha256.length += 512;
            md->sha256.curlen = 0;
         }
       }
...
/**
 * \brief  This function performs the last part of the SHA256 Hash process.
 *
 * \global_r  --
 * \global_w  --
 * \global_rw --
...
Crypto_ReturnType Crypto_SHA256Done(hash_state * md, uint8 *out)
{
  uint8 i;
  ...
  /* append the '1' bit */
  md->sha256.buf[md->sha256.curlen++] = (uint8)0x80;
  ...
  if (md->sha256.curlen > 56) {
      while (md->sha256.curlen < 64) {
          md->sha256.buf[md->sha256.curlen++] = (uint8)0;
      }
    Crypto_SHA256Compress(md, md->sha256.buf);
    md->sha256.curlen = 0;
  }
  ...
  /* store length */
  BUtil_Put64BitBigEndian((md->sha256.buf + 56), md->sha256.length);
  Crypto_SHA256Compress(md, md->sha256.buf);

  /* copy output */
  for (i = 0; i < 8; i++) {
    BUtil_Put32BitBigEndian(out+(4*i), md->sha256.state[i]);
  }
